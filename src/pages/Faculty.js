...
const emptyForm = { name: "", email: "", password: "", department: "", phone: "", role: "faculty" };
...
export default function Faculty() {
  ...
  const fetchFaculty = () => {
    setLoading(true);
    API.get("/users?role=faculty").then(res => { setUsers(res.data.users); setLoading(false); }).catch(() => setLoading(false));
  };
  useEffect(() => { fetchFaculty(); }, []);

  const openAdd = () => { setForm(emptyForm); setEditId(null); setError(""); setShowModal(true); };
  const openEdit = (u) => {
    setForm({ name: u.name, email: u.email, password: "", department: u.department || "", phone: u.phone || "", role: u.role });
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editId) {
        ...
        await API.post("/auth/register", { ...form, role: "faculty" });
      }
                    ...
                    <tr key={u._id}>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <button className="btn btn-ghost btn-sm" onClick={() => handleToggle(u._id)}>{u.isActive ? "🚫" : "✅"}</button>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="form-row">
                <div className="form-group" style={{ position: "relative" }}>
                  ...
                  <span onClick={() => setShowPassword(!showPassword)} style={{ position: "absolute", right: "10px", top: "34px", cursor: "pointer" }}>{showPassword ? "🙈" : "👁️"}</span>
                </div>
                <div className="form-group">
